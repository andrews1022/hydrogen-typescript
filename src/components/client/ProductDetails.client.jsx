...
export default function ProductDetails({ product }) {
  ...
  const sizeChartMetafield = productMetafields.find(
    (metafield) => metafield.namespace === 'my_fields' && metafield.key === 'size_chart'
  );
  const sustainableMetafield = productMetafields.find(
    (metafield) => metafield.namespace === 'my_fields' && metafield.key === 'sustainable'
  );
  const lifetimeWarrantyMetafield = productMetafields.find(
    (metafield) => metafield.namespace === 'my_fields' && metafield.key === 'lifetime_warranty'
  );
      ...
      <ProductProvider data={product} initialVariantId={initialVariant.id}>
        <div className='grid grid-cols-1 md:grid-cols-[2fr,1fr] gap-x-8 my-16'>
          <div className='md:hidden mt-5 mb-8'>
            ...
            {product.vendor && (
              <div className='text-sm font-medium mb-2 text-gray-900'>{product.vendor}</div>
            )}
            <span />
              ...
              <div className='flex items space-x-4'>
                {sustainableMetafield?.value && (
                  <span className='flex items-center mb-8'>
                    ...
                    <span className='text-sm text-gray-900 font-medium'>Sustainable Material</span>
                  </span>
                )}
                {lifetimeWarrantyMetafield?.value && (
                  <span className='flex items-center mb-8'>
                    ...
                    <span className='text-sm text-gray-900 font-medium'>Lifetime Warranty</span>
                  </span>
                )}
              </div>
            </div>
